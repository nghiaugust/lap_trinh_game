...
class Skeleton(
    ...
    private val speed = 100f // Increased speed for larger character size, still slower than heroes
    ...
    private var attackRange = 160f // Matching larger character size
    private var attackCooldown = 2000L // 2 seconds between attacks
    private var lastAttackTime = 0L
    private var detectionRange = 600f // Increased detection range for larger characters

    // Animation properties - optimized for performance
    private var currentFrame = 0
    private var animationTimer = 0f
    private val frameTime = 0.15f // Slower animation for better performance (was 0.1f)

    // Skeleton size - matching scaled hero size (hero sprite ~128x128 * 2x scale = ~256x256)
    private val skeletonWidth = 256f  // Same as scaled hero
    private val skeletonHeight = 256f // Same as scaled hero
    ...
    private fun generateNewPatrolTarget() {
        ...
        do {
            val angle = Random.Default.nextFloat() * 2 * PI.toFloat()
            val distance = Random.Default.nextFloat() * maxDistance
